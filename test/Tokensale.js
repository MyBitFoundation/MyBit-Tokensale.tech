...
contract('TokenSale', async (accounts) => {
  ...
  it('Withdraw from all 365 days', async() => {
    ...
    for (let i = 50; i < 365; i += 50){
      ...
      console.log("total myb owed to user 5 ", await tokenSale.getTotalTokensOwed(user5, batchDays));
      console.log("they should be owed ", tokensPer50Days);
      // assert.equal(bn(await tokenSale.getTotalTokensOwed(user5, batchDays)).eq(tokensPer50Days));
      batchDays = allDays.slice(350, 365);
      await tokenSale.batchWithdraw(batchDays, {from: user5});
    }
  });
